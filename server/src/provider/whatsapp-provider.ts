   ...
   */
    private async detectMessageTypeAndContent(
      ...
      if (type === "gp2") {
        ...
        switch (subtype) {
          case "revoke_invite":
            content = `${authorName} reset invitation link`;
            break;
          case "add":
          ...
          case "subject": {
            ...
            // Get old name (if any)
            const oldName = this.groupCache[cacheKey] || "(unknown)";
            content = `${authorName} changed the group name from "${oldName}" to "${newName}"`;
          
            // ✅ Update cache immediately for next time
            this.groupCache[cacheKey] = newName;
          
            // 🔄 Fire-and-forget update for better accuracy
            (async () => {
              try {
                const chat = await client.getChatById(chatId);
                if (chat?.name) {
                  this.groupCache[cacheKey] = chat.name;
                }
              } catch {
                // do nothing, just ignore failures
              }
            })();
          ...
          //   const cacheKey = `${accountId}:${chatId}`;
          //   oldName = this.groupCache?.[cacheKey] || oldName;

          //   // Fallback: ask client (will be new name)
          //   try {
          //     const chat = await client.getChatById(chatId);
          //     if (!oldName && chat?.name) oldName = chat.name;
          //   } catch {}

          //   content = `${authorName} changed the group name from "${oldName}" to "${newName}"`;

          //   // ✅ Update cache with new name
          //   if (this.groupCache) this.groupCache[cacheKey] = newName;
          //   content = `${authorName} changed the group name to"${body}"`;
          //   break;
          case "picture":
