  ...
  React.useEffect(() => {
    if (content) {
      ...
      if (isMediaUrl && primaryUrl) {
        ...
        // 🚀 立即检查文件是否已存在，如果存在则直接显示
        const testAudio = new Audio();
        testAudio.oncanplay = () => {
          console.log('✅ [语音初始化] 文件已存在，直接显示');
          setIsLoading(false);
          setHasError(false);
          setDownloadCompleted(true);
        };
        testAudio.onerror = () => {
          console.log('⏳ [语音初始化] 文件不存在，等待下载');
          // 保持loading状态，等待WebSocket通知
        };
        testAudio.src = primaryUrl;
      } else {
...
export function AllChatsView() {
  ...
  // 通用的滚动到底部函数
  const scrollToBottom = React.useCallback((immediate: boolean = false) => {
    setTimeout(() => {
...
React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (chats.length > 0 && !selectedChat) {
      ...
      if (firstChat.type === 'group' && firstChat.groupId) {
        const groupChats = chats.filter(c => c.groupId === firstChat.groupId)
  ...
  // 辅助函数：更新聊天列表中的聊天信息
  const updateChatInList = (chatId: string, groupId: string | undefined, updateFn: (chat: ChatInfo) => ChatInfo) => {
    setChats(prev => prev.map(chat => {
      const matchById = chat.id === chatId;
      const matchByGroup = !!(groupId && chat.type === 'group' && chat.groupId === groupId);
      return (matchById || matchByGroup) ? updateFn({ ...chat, updatedAt: Date.now() }) : chat;
    }));
  };

  // 辅助函数：检查是否为当前选中的聊天
  const isCurrentChat = (chatId: string, groupId: string | undefined) => {
    return !!selectedChat && (selectedChat.id === chatId || (!!groupId && selectedChat.type === 'group' && selectedChat.groupId === groupId));
  };

  // 辅助函数：更新当前选中的聊天
  const updateCurrentChat = (updateFn: (chat: ChatInfo) => ChatInfo) => {
    if (selectedChat) {
      setSelectedChat(prev => prev ? updateFn({ ...prev, updatedAt: Date.now() }) : prev);
    }
  };

  // 辅助函数：添加系统消息
  const addSystemMessage = (chatId: string, content: string, messageId: string) => {
  ...
  // 辅助函数：获取操作者信息
  const getChangedBy = (data: any) => {
    return data?.changedBy || data?.actionDetails?.changedBy || 'Someone';
  };

  // 监听群组操作事件
  React.useEffect(() => {
    const handleGroupNameChanged = (event: CustomEvent) => {
      console.log('📝 [WS->UI] group_name_changed:', event.detail);
      ...
      updateChatInList(chatId, groupId, chat => ({ ...chat, name: newName || chat.name }));
      
      if (isCurrentChat(chatId, groupId)) {
        updateCurrentChat(chat => ({ ...chat, name: newName || chat.name }));
        
        const changedBy = getChangedBy(data);
        const content = oldName && oldName !== 'Unknown'
          ? `${changedBy} renamed the group from "${oldName}" to "${newName}"`
          : `${changedBy} renamed the group to "${newName}"`;
        
        addSystemMessage(chatId, content, `system-group-name-${Date.now()}`);
      }
    };

    const handleGroupPhotoChanged = (event: CustomEvent) => {
      console.log('🖼️ [WS->UI] group_photo_changed:', event.detail);
      const data: any = event.detail;
      const chatId = data?.chatId || data?.id;
      const groupId = data?.groupId;
      const newPhotoUrl = data?.newPhotoUrl || data?.avatar;

      updateChatInList(chatId, groupId, chat => ({ ...chat, avatar: newPhotoUrl || chat.avatar }));
      
      if (isCurrentChat(chatId, groupId)) {
        updateCurrentChat(chat => ({ ...chat, avatar: newPhotoUrl || chat.avatar }));
        
        const changedBy = getChangedBy(data);
        addSystemMessage(chatId, `${changedBy} changed the group photo`, `system-group-photo-${Date.now()}`);
      }
    };

    const handleGroupMemberAdded = (event: CustomEvent) => {
      console.log('➕ [WS->UI] group_member_added:', event.detail);
      const data: any = event.detail;
      const chatId = data?.chatId || data?.id;
      const groupId = data?.groupId;
      const addCount = (data?.addedUsers?.length || 1);
      const addedUsers = data?.addedUsers || [];

      updateChatInList(chatId, groupId, chat => ({ 
        ...chat, 
        memberCount: (chat.memberCount || 0) + addCount 
      }));
      ...
      if (isCurrentChat(chatId, groupId)) {
        ...
        const content = addedUsers.length > 0 
          ? `${addedUsers.map((u: any) => u.name || u.username || 'Unknown').join(', ')} joined the group`
          : `${addCount} members joined the group`;
        
        addSystemMessage(chatId, content, `system-member-added-${Date.now()}`);
      }
    };

    const handleGroupMemberRemoved = (event: CustomEvent) => {
      console.log('➖ [WS->UI] group_member_removed:', event.detail);
      const data: any = event.detail;
      const chatId = data?.chatId || data?.id;
      const groupId = data?.groupId;
      const removedUser = data?.removedUser;
      ...
      if (isCurrentChat(chatId, groupId)) {
        ...
        const content = removedUser 
          ? `${removedUser.name || removedUser.username || 'Unknown'} left the group`
          : 'A member left the group';
        
        addSystemMessage(chatId, content, `system-member-removed-${Date.now()}`);
      }
    };

    const handleMessagePinned = (event: CustomEvent) => {
      console.log('📌 [WS->UI] message_pinned:', event.detail);
    };

    const handleGroupAction = (event: CustomEvent) => {
      console.log('🔄 [WS->UI] group_action:', event.detail);
      const data: any = event.detail;
      const chatId = data?.chatId || data?.id;
      const groupId = data?.groupId;

      updateChatInList(chatId, groupId, chat => chat);
    ...
    const handleGroupUpdated = (event: CustomEvent) => {
      console.log('🔄 [WS->UI] group_updated:', event.detail);
      const data: any = event.detail;
      const chatId = data?.chatId || data?.id;
      const groupId = data?.groupId;
    ...
    // 注册事件监听器
    const eventHandlers = [
      { event: 'chatx:group_name_changed', handler: handleGroupNameChanged },
      { event: 'chatx:group_photo_changed', handler: handleGroupPhotoChanged },
      { event: 'chatx:group_member_added', handler: handleGroupMemberAdded },
      { event: 'chatx:group_member_removed', handler: handleGroupMemberRemoved },
      { event: 'chatx:message_pinned', handler: handleMessagePinned },
      { event: 'chatx:group_action', handler: handleGroupAction },
      { event: 'chatx:group_updated', handler: handleGroupUpdated }
    ];
